...
public interface AgendamentoRepository extends JpaRepository<Agendamento, Long>{
 ...
 //Retorna os horários q não estão presentes na tabela de Agendamento (subquery), ou seja, horários disponíveis
	@Query("select h "
			+ "from Horario h "
			+ "where not exists("
				+ "select a.horario.id "
					...
					+ "where "
						...
						+ "a.horario.id = h.id "			//Verifica o horário especificado
			+ ") "
			+ "order by h.horaMinuto asc")
	List<Horario> findByMedicoIdAndDataNotHorarioAgendado(Long id, LocalDate data);

	//Faz projeção com as colunas correspondentes
	//CONCAT é do sql, como está concatenando, o seu retorno é uma String
